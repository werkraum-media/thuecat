...
- **New transient field**: extend the relevant ``Entity::parse()`` to push refs into the payload's ``transients`` bucket; teach ``Resolver`` to drain the new key. This covers a **scalar** reference — one FK on the owner row. A property that produces a *set* of relations follows a different shape; see `Relation-set properties`_.
